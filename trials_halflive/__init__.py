...
Trials are pregenerated and embedded into page via js_vars.
Iteration runs on-page.
Answers and feedback are communicated with server.
...
class Main(Page):
    ...
    def js_vars(player: Player):
        ...
        answered = list(filter(lambda t: t.success is not None, trials))
        if len(answered) > 0:
            current = dict(iteration=answered[-1].iteration, total_score=player.total_score)
        else:
            current = None

        return dict(
            trials=list(map(format_trial, trials)),
